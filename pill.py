...
class pillPhot:
    ...
    def __call__(self,xi,yi,radius=4.,l=5.,a=0.01,width=20.,skyRadius=8.,zpt=27.0,exptime=1.,
        ...
        self.trimBGHighPix = trimBGHighPix
...
#        if angle > 90 or angle < -90 or length < 0 or num.min(radius) < 0:
        ...
        if skyRadius == None:
            ...
            if not trimBGHighPix:
                ...
                #below doesn't display any frames, just sets up the display window, and does the BG trimming
                if display and enableBGSelection:
                    bgf.plotAxis = self.dispFig.add_subplot(self.dispGS[1])
                bg = bgf.smartBackground(display=display, backupMode=backupMode, forceBackupMode = forceBackupMode)
        ...
        if display:
            if trimBGHighPix:
                #this is necessary for display purposes, but doesn't actually adjust the background measure which uses self.data, not im
    ...
    def _on_ylims_change(self, axis):
        ...
        w = num.where(junk<>0)
        #need to put a BG trim here!
        if self.trimBGHighPix:
            w = num.where((junk>0)&(junk<self.bg+ self.trimBGHighPix * self.bgstd))
        else:
            w = num.where(junk<>0)
